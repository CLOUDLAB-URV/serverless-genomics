...
#file1_name="${file1%.*}"
# paired end example
# bash gempileup_v8.sh /home/lumar/bioss/cloudbutton/scripts/SRR6052133_split_12_filt_wline_no_corrected.map /home/lumar/bioss/test_data/Tbrucei/fasta-chunks/TriTrypDB-9.0_TbruceiTREU927_Genome_MbChr_split_1.fasta paired-end True
...
seq_type="${3:-/dev/stdin}"
debug="${4:-"False"}"
#echo "gempileup_v7.sh starting"
awk -v NAME="$map_file" -v ref_genome="$ref_genome" -v seq_type="$seq_type" -v debug="$debug" '
...
#function process_mutation_indel(read_info,gigar_index,strand,seq,qual,gig_elem,base_count,start,base_out,abs_bpos,read_start,ins,del,ins_prev,del_prev,ins_cumul,del_cumul,ins_cumul_prev,del_cumul_prev,chr,read_len) 
function process_mutation_indel(seq,gig_elem) 
{
    ...
    if (strand=="+") {
        ...
        base_qual = substr(qual,base_count+ins_cumul,1)
    }
    else {
    ...
    printf ("%s\t%s\t%s\t%s\t%s\t%s\t", chr, abs_bpos, ref_bp, "1", base_out_mut, base_qual)
...
#function process_number_indel(read_info,gigar_index,strand,seq,qual,gig_elem,base_count,start,base_out,abs_bpos,read_start,ins,del,ins_prev,del_prev,ins_cumul,del_cumul,ins_cumul_prev,del_cumul_prev,chr,read_len) 
...

{
    if (gig_elem!~/>/) {
        for (m = 1; m <= gig_elem; m++) {
            ...
            base_qual = substr(qual,rel_bpos,1)
        ...
        for (m = 1; m <= match_elem; m++) {
            ...
            # print mpileup line
            printf_debug(debug,"%s\t%s%s\t", read_info, gigar_index, strand) 
            printf ("%s\t%s\t%s\t%s\t%s\t%s\t", chr, abs_bpos, ref_bp, "1", base_out, base_qual)
            print_newline_no_debug(debug)
            print_debug(debug, "read_start: "read_start"; rel_bpos: "rel_bpos"; base_count: "base_count"; m: "m"; del: "del"; ins: "ins"; del_prev: "del_prev"; ins_prev: "ins_prev"; del_cumul: "del_cumul"; ins_cumul: "ins_cumul"; del_cumul_prev: "del_cumul_prev"; ins_cumul_prev: "ins_cumul_prev)
...
if (count_reads >= 1 && aln_col!="-") {
#if (count_reads >=1 && count_reads <=23000 && aln_col!="-") {
#if ($0~"SRR15068323.9174_") {
    print_debug(debug, "####\n##read "count_reads)
    print_debug(debug,"read_name: "read_name)
    print_debug(debug,"read: "read)
    print_debug(debug,"qual: "qual)
    print_debug(debug,"summary: "summary)
    print_debug(debug,"aln_col: "aln_col)

    read_len = length(seq1)
    print_debug(debug,"read length: "read_len)

    # if alignments are on minus strand, create complement (no need to reverse qual)
    compl_seq1=""
    compl_seq2=""
    if (aln_col~":-:") { # check for minus strand in alignment field
        compl_seq1 = complement(seq1)
        if (seq_type == "paired-end") {
            compl_seq2 = complement(seq2)
        }
    }
    
    # SPLIT ALIGNMENTS FIELD INTO INDIVIDUAL ALIGNMENTS
    align_n = split(aln_col, aligns, /,/)
    
    print_debug(debug,"number of alignments: "align_n)
    for (f = 1; f <= align_n; f++) {
        print_debug(debug, "alignment no. "f": "aligns[f])
        # if paired end, need to split alignment line into two (read1 and read2)
        # do the split also for single-end, and then use the first part of the split
        split(aligns[f], paired_align, /::/)

        # split each alignment into columns (chr, strand, read_start, gigar)
        # read1 (or only read if single end)

        split(paired_align[1], align_cols1, /:/)
        chr1=align_cols1[1]
        strand1=align_cols1[2]
        read_start1=align_cols1[3]
        gigar1=align_cols1[4]
        if (seq_type="paired-end")
            split(paired_align[2], align_cols2, /:/)
            chr2=align_cols1[1]
            strand2=align_cols1[2]
            read_start2=align_cols1[3]
            gigar2=align_cols1[4]
        ...
        if (seq_type="paired-end" && gigar1~/>[0-9]+[\+-]>[0-9]+[\+-]/) {
            continue
        }
