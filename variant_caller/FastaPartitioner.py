        ...
        while i < total_chunks+1:
            ...
            for el in data_lines:
                if 'x' in el:
                    ...
                    chunk = self.storage.get_object(self.obj.bucket, self.obj.key,stream=True,extra_get_args = {'Range': f'bytes={start}-{int(end)-1}'}).read().decode('utf-8')
...
if __name__ == "__main__":
    ...
    log_level='DEBUG',runtime=runtime, max_workers=1000,runtime_memory=4096,backend='aws_lambda')

    """
    obj = 's3://tyler-ddrad/fasta/hg19.fa'  # Change-me
    my_bucket_name = 'tyler-ddrad' # Change-me
