...
class MapReduce:
    ...
    def __init__(self, map_func, reduce_func, create_sinple_key, runtime, runtime_memory, runtime_memory_r, buffer_size, file_format, func_timeout_map, func_timeout_reduce,log_level, bucket, stage, id, debug, skip_map, EC2_IP, workers, method, PEM, fastq_set_n, run_id, iterdata_n, redis_index_correction):
    ...
    def __call__(self, iterdata, iterdata_sets):
        ...
        fexec = lithops.FunctionExecutor(runtime=self.runtime,runtime_memory=self.runtime_memory,log_level=self.log_level, max_workers=self.workers)
        ...
        if self.skip_map == "False":
            if not iterdata_sets:
                ...
                if self.redis_index_correction==True:
                    ec2.start_redis_reducer(path_to_secret_key=self.PEM, ec2_IP_address=self.EC2_IP, fastq_set_n=self.fastq_set_n, run_id=self.run_id, iterdata_map_n=self.iterdata_n)
                    time.sleep(5)
                # print("self.map_func: "+str(self.map_func))
                # print("iterdata: "+str(iterdata))
                # print("timout: "+str(self.func_timeout_map))
                fexec.map(self.map_func, iterdata, timeout=int(self.func_timeout_map))
        ...
        if(self.method == "select"):
            ...
            fexec = lithops.FunctionExecutor(runtime=self.runtime, runtime_memory=8192, log_level=self.log_level)
        ...
        ############
            ...
            fexec = lithops.FunctionExecutor(runtime=self.runtime, runtime_memory=4096, log_level=self.log_level)
        ...
        fexec = lithops.FunctionExecutor(runtime=self.runtime,runtime_memory=self.runtime_memory_r, log_level=self.log_level)
