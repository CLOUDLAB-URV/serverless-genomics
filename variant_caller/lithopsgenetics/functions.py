...
def fastq_to_mapfun(fastq_n, fastq_file_key, fastq_chunk_data, BUCKET_NAME, fastq_folder, idx_folder, datasource,stage,id,debug):
    ...
    if datasource == None:
        ...
        temp_fastq_gz = af.copy_to_runtime(storage, BUCKET_NAME, fastq_folder, fastq_file_key, byte_range)
        ...
        temp_fastq_i = af.copy_to_runtime(storage, BUCKET_NAME, idx_folder, f'{fastq_file_key}i')
        ...
        wd = os.getcwd()
        os.chdir("/tmp")
        temp_fastq = '/tmp/'+seq_name+f'_chunk{fastq_chunk_data["number"]}.fastq'
        data_output = sp.run(['fastq-dump', str(seq_name), '-X', str(int(fastq_chunk_data["start_line"])) , '-N', str(int(fastq_chunk_data["end_line"])),'-O','/tmp'],
                             capture_output=True)
