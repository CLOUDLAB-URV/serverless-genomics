...
then
    ...
    /bin/gem3-mapper/bin/gem-mapper -I "$gem_ref" -i "$fastq" -F MAP | bash /home/lumar/bioss/serverless_genomics/variant_caller/dockerfile/parse_gem_maxindex_minimapfile_stdin_v2.sh  /dev/stdin $map_file_nosuffix $seq_type
else
    echo "paired-end alignment"
    /bin/gem3-mapper/bin/gem-mapper -I "$gem_ref" -1 "$fastq" -2 "$fastq2"  -F MAP | bash /home/lumar/bioss/serverless_genomics/variant_caller/dockerfile/parse_gem_maxindex_minimapfile_stdin_v2.sh  /dev/stdin $map_file_nosuffix $seq_type
fi
