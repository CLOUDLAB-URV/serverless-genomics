...
parser.add_argument('--data_location', 
                    help='Location of the fasta file, ex: s3://ayman-lithops-meta-cloudbutton-hutton/fasta/hg19.fa')
        ...
        while i < total_chunks+1:
            ...
            for el in data_lines:
                if 'x' in el:
                    ...
                    chunk = self.storage.get_object(self.obj.bucket, self.obj.key,stream=True,extra_get_args = {'Range': f'bytes={start}-{end-1}'}).read().decode('utf-8')
...
def generate_chunks_corrected_index(id,r,storage):
    ...
    obj_bucket = location.split('/')[0]
    obj_key = location.split('/')[1:]
    obj_key = '/'.join(obj_key)
    obj_name = location.split('/')[-1].split('.')[0]
    start=0
    end=0
    ...
    for el in ret_data.splitlines():
        if ',' in el and '-' in el:
            ...
            data = data + str(storage.get_object(obj_bucket, obj_key, stream=True,extra_get_args = {'Range': f'bytes={start}-{end}'}).read().decode('utf-8')) + '\n'
    storage.put_object(my_bucket_name, f'{fasta_folder}/{obj_name}_{worker_chunk_size}split{id}.fasta',data)
    data = ""
...
if __name__ == "__main__":
    fexec = lithops.FunctionExecutor(
        log_level='DEBUG',runtime='testcontainer',max_workers=1000,runtime_memory=4096,backend='aws_lambda')
    ...
    fexec.map(run_worker_metadata, {'my_bucket_name':my_bucket_name,'obj':obj,'total_obj_size':int(total_fasta_size),'obj_size':fasta['content-length'], 'overlap':ovlp},
            obj_chunk_size=worker_chunk_size)
    fexec.wait()
    iterdata = fexec.get_result()
    
    #First mapper doesn't always need to be executed (Caching features)
    fexec.call_async(generate_index_file,{'my_bucket_name':my_bucket_name,'obj':obj,'total_obj_size':int(total_fasta_size)})
    fexec.wait()
