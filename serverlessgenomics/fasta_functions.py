...
def prepare_fasta(args: PipelineParameters):
    ...
    try:
        ...
        create_fastafile_index(args.bucket, args.fasta_folder, args.fasta_file, args.fasta_folder_index,
                               args.fasta_workers)
        try:
...
def create_fastafile_index(bucket_name, fasta_folder, fasta_file, fasta_folder_index, fasta_n_workers):
    ...
    # prints all files
    args = "python fastaPartitionerIndex.py --key " + fasta_folder + fasta_file + " --workers " + str(
        fasta_n_workers) + " --mybucket " + bucket_name + " --fasta_folder " + fasta_folder_index
