        ...
        if ini_heads or data[0] == '>':  # If the list is not empty or there is > in the first byte
            ...
            for m in heads:
                ...
                if first_sequence:
                    ...
                    if id > 0 and start - 1 > min_range:  # If it is not the worker of the first part of the file and 
                        # in addition it turns out that the partition begins in the middle of the base of a sequence.
                        ...
                        else:  # When the first header found is false, when in a split stream there is a split header that has 
                               # a '>' inside (ex: >tr|...o-alpha-(1->5)-L-e...\n)
                            first_sequence = True
