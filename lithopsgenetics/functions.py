...
# Modified, def split_fastafile(cloud_adr, runtime, bucket_name, fasta_folder, fasta_file, split_fasta_folder, fasta_chunk_size, chunk_overlap):
def create_fastafile_index(bucket_name, fasta_folder, fasta_file, fasta_folder_index, fasta_n_workers):
    ...
    # data = storage.get_object(bucket=bucket_name, key='fasta-indexs/TriTrypDB-9.0_TbruceiTREU927_Genome_MbChr.fai').decode('utf-8')
    ...
    # Modified, print("end of *split_fasta* function")
...
def generate_fastq_chunk_list(bucket_name, index_folder, fastq_file_name, fastq_lines, seq_type, fastq_file_name2=None):
    ...
    sp.run(CURRENT_PATH+'/generateChunks.sh '+fastq_file_name+' '+block_length+' '+total_lines, shell=True, check=True, universal_newlines=True)
...
def generate_alignment_iterdata(BUCKET_NAME, list_fastq, fasta_index, fasta_file_path, iterdata_n):
    """
...
# Modified, def prepare_fasta(cloud_adr, runtime, BUCKET_NAME, fasta_folder, fasta_file, fasta_folder_index, fasta_chunk_size, fasta_chunks_prefix, fasta_line_overlap):
def prepare_fasta(BUCKET_NAME, fasta_folder, fasta_file, fasta_folder_index, fasta_n_workers):
    storage = Storage()
    fasta_index = fasta_folder_index + pathlib.Path(fasta_file).stem + f'_{fasta_n_workers}.fai'
    try:
        # Modified, print("Searching " + BUCKET_NAME + "/" + fasta_folder_index + fasta_chunks_prefix)
        print("Searching " + BUCKET_NAME + "/" + fasta_index)
        # Modified, print("Finding in storage: \n" + str(storage.list_keys(BUCKET_NAME, prefix=fasta_folder_index + fasta_chunks_prefix)))
        list_files_index = storage.list_keys(BUCKET_NAME, prefix=fasta_folder_index)
        print("Finding in storage: \n" + str(list_files_index))
        # fasta_list = storage.list_keys(BUCKET_NAME, prefix=fasta_folder_index) # Modified, fasta_list = storage.list_keys(BUCKET_NAME, prefix=fasta_folder_index + fasta_chunks_prefix)
        storage.head_object(BUCKET_NAME, fasta_index)
        ...
        create_fastafile_index(BUCKET_NAME, fasta_folder, fasta_file, fasta_folder_index, fasta_n_workers)
        try:
            fasta_index = fasta_folder_index + pathlib.Path(fasta_file).stem + '.fai'
            # Modified, print("Finding newly created fasta chunks")
            # Modified, print("Searching " + BUCKET_NAME + "/" + fasta_folder_index + fasta_chunks_prefix)
            print("Searching " + BUCKET_NAME + "/" + fasta_folder_index)
            # Modified, print("Finding in storage: \n" + str(storage.list_keys(BUCKET_NAME, prefix=fasta_folder_index + fasta_chunks_prefix)))
            print("Finding in storage: \n" + str(storage.list_keys(BUCKET_NAME, prefix=fasta_folder_index)))
            # Modified, fasta_list = storage.list_keys(BUCKET_NAME, prefix=fasta_folder_index + fasta_chunks_prefix)
            # Modified, fasta_list = storage.list_keys(BUCKET_NAME, prefix=fasta_folder_index)
            storage.head_object(BUCKET_NAME, fasta_folder_index+fasta_file)
            fasta_index = fasta_folder_index+pathlib.Path(fasta_file).stem+'.fai'
            #print("fasta list: " + str(fasta_list))
        except:
