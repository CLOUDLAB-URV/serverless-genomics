...
def create_fasta_chunk_for_runtime(storage, bucket, fasta, byte_range, folder, file_name):
...
def copy_to_runtime(storage, bucket, folder, file_name, byte_range={}, key=None, fasta=None):
    printl(f'Copying {file_name} to /tmp folder')
...
def copy_to_s3(storage, BUCKET_NAME, file_name, temp_to_s3, folder_name=""):
    if temp_to_s3==True:
        printl("\ncopying "+file_name+" to "+BUCKET_NAME)

        destination_key= folder_name + os.path.basename(file_name)
        printl("destination key: "+destination_key)
