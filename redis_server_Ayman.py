...
def group_by_fasta(file_list):
    splits = []
    for i in file_list:
        splits.append(i.split('_')[3])
    splits = list(dict.fromkeys(splits))
    return(splits)

def execute_binary_reduce(splits,redis):
    for split in splits:
        print('./binary_reducer merge_gem_alignment_metrics.sh 4 '+split+'* >  '+split+'.intermediate.txt')
        cmd = f'./binary_reducer.sh /home/ubuntu/merge_gem_alignment_metrics.sh 4 {split}* > {split}.intermediate.txt'
...
if __name__ == "__main__":
    ...
    while (True):
        ...
        if len(file_list) > 0:
            ...
            execute_binary_reduce(splits,redis)
        time.sleep(0.3)
